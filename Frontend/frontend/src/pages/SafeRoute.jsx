...
export default function SafeRoute() {
  ...
  const findSafeRoute = async () => {
    ...
    try {
      const rawRoutes = await fetchRoutes(origin, destination, profile);

      const allPoints = rawRoutes.flatMap((r) => r.path);
      const nearbyServices = await fetchEmergencyServices(allPoints);

      const scored = scoreRoutes(rawRoutes, nearbyServices);
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="inline-flex rounded-xl border border-zinc-800 overflow-hidden">
              <button
                ...
                className={`px-3.5 py-2 text-xs font-bold flex items-center gap-1.5 transition-all ${
                  profile === "driving" ? "bg-emerald-600 text-white" : "text-zinc-400 hover:text-white"
                }`}
              >
                <Car size={13} /> Driving
              </button>
              <button
                ...
                className={`px-3.5 py-2 text-xs font-bold flex items-center gap-1.5 transition-all border-l border-zinc-800 ${
                  profile === "foot" ? "bg-emerald-600 text-white" : "text-zinc-400 hover:text-white"
                }`}
              >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`grid h-8 w-8 place-items-center rounded-lg ${
                        s.type === "police" ? "bg-blue-500/10 text-blue-400" : "bg-rose-500/10 text-rose-400"
                      }`}
                    >
